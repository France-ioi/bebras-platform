...
function handleGroupFromRegistrationCode($db, $code) {
   ...
   if (isset($_POST["startOfficial"])) {
      $contestID = "220445039788370339"; // hard-coded real contest
...
function handleCheckGroupPassword($db, $password, $getTeams, $extraMessage = "", $registrationData = null, $isOfficialContest = false) {
   ...
   $_SESSION["groupClosed"] = (($nbMinutesElapsed > intval($row->groupsExpirationMinutes) + 30) && (intval($row->groupsExpirationMinutes) != 0) && (!$_SESSION["isPublic"]) && (!$_SESSION["isGenerated"]));
